...
void fail(int number, char * message) {
...
image_data * setup() {
    ...
    unsigned char ** pixels1 = read_image("test_image.png", &height, &width, &channels, &bit_depth, &color_type);
    image_data * image_p = initialise_data(pixels1, height, width, 3);
    return image_p;
}

void teardown(image_data * image_p) {
