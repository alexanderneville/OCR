...
typedef struct
{
  PyObject_HEAD PyObject *infile_path;
...
static PyMemberDef Pipeline_members[] = {
  {"infile_path", T_OBJECT_EX, offsetof(PipelineObject, infile_path), 0,
   "the input file"},
  {"outfile_path", T_OBJECT_EX, offsetof(PipelineObject, outfile_path), 0,
   "the output file"},
  {"dataset_path", T_OBJECT_EX, offsetof(PipelineObject, dataset_path), 0,
   "path for the full dataset"},
  {"sample_path", T_OBJECT_EX, offsetof(PipelineObject, sample_path), 0,
   "path for the samples"},
  {"info_path", T_OBJECT_EX, offsetof(PipelineObject, info_path), 0,
   "path for general info"},
  {"height", T_INT, offsetof(PipelineObject, height), 0,
   "height of the image in pixels"},
  {"width", T_INT, offsetof(PipelineObject, width), 0,
   "width of the image in pixels"},
  {"channels", T_INT, offsetof(PipelineObject, channels), 0,
   "number of channels"},
  {"color_type", T_INT, offsetof(PipelineObject, color_type), 0,
   "refer to libpng documentation"},
  {"bit_depth", T_INT, offsetof(PipelineObject, bit_depth), 0,
...
{
  static char *kwlist[] =
    { "infile_path", "outfile_path", "dataset_path", "sample_path",
"info_path", NULL };
  PyObject *infile_path = NULL, *outfile_path = NULL, *dataset_path =
    NULL, *sample_path = NULL, *info_path = NULL, *tmp;

  if (!PyArg_ParseTupleAndKeywords
      (args, kwds, "|OOOOO", kwlist, &infile_path, &outfile_path,
       &dataset_path, &sample_path, &info_path))
    return -1;
  if (infile_path) {
...
Pipeline_files(PipelineObject * self, PyObject * Py_UNUSED(ignored))
{
  ...
  return PyUnicode_FromFormat("%S %S %S %S %S", self->infile_path,
                              self->outfile_path, self->dataset_path,
                              self->sample_path, self->info_path);
}
...
Pipeline_load_file(PipelineObject * self, PyObject * args)
{
  ...
  unsigned char **pixels =
    read_image(infile_path, &(self->height), &(self->width),
               &(self->channels), &(self->bit_depth), &(self->color_type));

  self->image =
    initialise_data(pixels, self->height, self->width, self->channels);
  return PyLong_FromLong(1);
}
...
Pipeline_save_to_file(PipelineObject * self, PyObject * args)
{
  ...
  if (self->image == NULL) {
    PyErr_SetString(Pipeline_Error,
                    "Call to 'save_to_file' must follow 'load_file'.");
    return NULL;
  }
  unsigned char **pixels = self->image->export_pixels(self->image);

  write_image(outfile_path, pixels, self->image->height, self->image->width,
              self->image->channels, self->bit_depth, self->color_type);
  return PyLong_FromLong(1);
}
...
Pipeline_generate_dataset(PipelineObject * self, PyObject * args)
{
  ...
  self->image->generate_dataset_from_image(self->image, dataset_path,
                                           sample_path, info_path);
  return PyLong_FromLong(1);
}
...
static PyObject *
Pipeline_switch_channel_num(PipelineObject * self,
...
static PyModuleDef custommodule = {
  ...
  .m_name = "pipeline",
  .m_doc =
    "extension type for interfacing with the image processing pipeline.",
  .m_size = -1,
};
