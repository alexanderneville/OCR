...
void
traverse_tree(tree_node * node_p, image_data * image)
{
  if (node_p->left) {
    traverse_tree(node_p->left, image);
  }
  image->greyscale->array[node_p->data->position] = node_p->data->intensity;
  if (node_p->right) {
    traverse_tree(node_p->right, image);
  }
}

void
destroy_tree(tree_node * node_p)
{
  if (node_p->left) {
    destroy_tree(node_p->left);
  }
  if (node_p->right) {
    destroy_tree(node_p->right);
  }
  free(node_p->data);
  free(node_p);
}
