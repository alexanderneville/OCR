...
class FC_Dense_Layer(Base_Layer):
    ...
    def __init__(self, num_inputs, num_outputs):
        ...
        * num_outputs corresponds to the number of pereptons in this layer.
        ...
        Used together these variables can determine how many weights are needed.
        """
        ...
        # subtract 0.5 from each number to get a random distribution from -0.5 to 0.5
        self._weights = (np.random.rand(num_inputs, num_outputs) - 0.5)
        self._biases = (np.random.rand(1, num_outputs) - 0.5)

    def feed_forward(self, input_data):
        """
        Given a vector containing all of the outputs of the previous layer,
        multiply by all of the weights leading to this layer.
        'inputs' is a matrix with one column and many rows.
        The shape is therefore == (num_inputs, 1)
        """
        # record the inputs for use during backprop
        self._input_data = input_data
        # multiply by weights
        output = np.dot(self._input_data, self._weights)
        # add the biases to the outputs
    ...
    def layer_parameters(self):
        """ Return the tunable parameters for this layer. """
    ...
    def update(self, d_weights, d_biases):
        """ Update parameters by given vectors used during mini-batch gradient descent. """
