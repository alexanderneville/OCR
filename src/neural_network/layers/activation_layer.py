...
class Activation_Layer(Base_Layer):
    ...
    def feed_forward(self, input_data):
        """
        Like other layer, this function is called as data is fed through the network.
        Therefore the forward function takes in a row vector and returns a vector of the same shape.
        """

        # record the input for backpropagation.
    ...
    def propagate_backward(self, layer_error, learning_rate):
        """
        This layer has no trainable parameters so no adjustments to the layer have to be made. This function simply
        returns dE/dX given dE/dY. The learning rate has to be included so that all layers have a common interface
        even though it is not needed in activation layers.
        """
