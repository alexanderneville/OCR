...
def simulate_number_labelling(file):
    ...
    for i in range(len(data["characters"])):
        data["characters"][i]["label"] = "NEAtestvideo!"[i]
...
def intro_test():
    ...
    simulate_number_labelling("intro")
    # load the data needed to train a new model
    data = get_info("./output/intro_info.json")
    extended_dataset, sample_dataset = get_datasets(data, "./output/intro_dataset.txt",
                                                    "./output/intro_sample.txt")
    ...
    network.export_layout("./models/intro_model.json")
    del(network)
    network = nn.Network.import_layout("./models/intro_model.json")
