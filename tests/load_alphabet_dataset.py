...
#     visualise_inputs(characters_to_display)
    ...
    y_values = [i for i in "abcdefghijklmnopqrstuvwxyz"]

    x_dataset = []
    y_dataset = []

    # only valid because the letters appear in order in the dataset
    ...
    network = nn.Network([32*32, 75, 75, 26])
    network.train(x_dataset, y_activations, 4000, 0.15)
    y_predicted = network.predict(x_dataset)
    evaluate(y_predicted, y_dataset, y_values)

    network.export_layout("data/alphabet_model.json")
    new_network = nn.Network.import_layout("data/alphabet_model.json")
